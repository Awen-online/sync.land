...
function fml_api_analytics_survey_submit(WP_REST_Request $request) {
    ...
    $upload_ease = isset($body['upload_ease']) ? intval($body['upload_ease']) : null;
    if ($upload_ease !== null && ($upload_ease < 1 || $upload_ease > 5)) {
        $upload_ease = null;
    }

    $role = isset($body['role']) ? sanitize_text_field($body['role']) : null;
    if (!in_array($role, ['licensee', 'artist', 'both'], true)) {
        $role = null;
    }
    ...
    $artist_focus = '';
    if (isset($body['artist_focus']) && is_array($body['artist_focus'])) {
        $artist_focus = implode(',', array_map('sanitize_text_field', array_slice($body['artist_focus'], 0, 10)));
    } elseif (isset($body['artist_focus'])) {
        $artist_focus = sanitize_text_field(substr($body['artist_focus'], 0, 255));
    }
    ...
    $insert_data = [
        ...
        'artist_focus'   => $artist_focus ?: null,
...
function fml_api_analytics_export(WP_REST_Request $request) {
    ...
    if ($type === 'survey') {
        $table = $wpdb->prefix . 'fml_survey_responses';
        $columns = ['id', 'session_id', 'user_id', 'role', 'nps_score', 'use_case', 'artist_focus', 'licensing_ease', 'upload_ease', 'feature_request', 'how_found_us', 'trigger_type', 'page_url', 'created_at'];
    } else {
